...
const EditItem = () => {
  ...
  useEffect(() => {
    (async () => await fetch(`http://localhost:8070/item/get/${id}`).then((res) => res.json()).then(json => setNewUser(json)))()
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`http://localhost:8070/item/update/${id}`, formData) //update Item data
      .then(res => {
        console.log(res);
        setLoading(false);
    ...
    <div>
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          <ul className="navbar-nav me-auto mb-2 mb-lg-0 nav nav-tabs">
            ...
            <li className="nav-item">
                <Link className="nav-link active" to="#" style={{ color: "#008080" }}><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit Item</Link>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="marketingImage" style={{ width: "100%" }}  ><br /><br /><br />
        <form onSubmit={handleSubmit} encType='multipart/form-data' style={{ width: "35%", marginLeft: "auto", marginRight: "auto", display: "block", background: "#ccffff", padding: " 10px 10px 10px 10px", opacity: "0.8" }}>
          <div className="cmb-3"><br />
            ...
            /><br />
            <label for="item_Id" className="form-label" style={{color:"black"}}>Item Id <span class="required">*</span></label>
            <input
             style={{width:"70%"}}
              type="text"
